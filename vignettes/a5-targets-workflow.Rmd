---
title: "A5 - Targets Workflow"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{A5 - Targets Workflow}
...
analysis. The template is shipped under `inst/templates/targets/`; it is not a
package-root `_targets.R` file. Copy it into a private analysis project before
running it.
...
The real-data manifest encoded in the template follows the v0.6.0 coverage
policy: budget covers 2021-22 through 2024-25; classroom and student cover
2021-22 through 2025-26; the 2025-26 budget is structurally unavailable and is
not zero-filled.
...
Copy the template into a private working folder. Do not run real-data workflows
...
```{r install-targets, eval = FALSE}
install.packages("targets")
```

DuckDB output also requires the optional database packages:

```{r install-duckdb, eval = FALSE}
install.packages(c("duckdb", "DBI"))
```

## Synthetic Mode

Synthetic mode is the default and is safe for package checks, teaching, and
workflow rehearsal.

```{r synthetic-env, eval = FALSE}
Sys.setenv(
...
```{r tar-make-synthetic, eval = FALSE}
targets::tar_make()
```
...
In synthetic mode, row-level RDS and DuckDB outputs are allowed by default
because the data are fabricated.

## Real-Data Mode

Real-data mode is explicit. It requires a private source-data folder and a
private output folder.

```{r realdata-env, eval = FALSE}
Sys.setenv(
...
`ALPREKDB_DATA_DIR` does not exist or if canonical source files are missing.
...
- row-level RDS outputs and DuckDB writes are disabled unless explicitly
  enabled;
- public documentation should report aggregate diagnostics, not row-level
  records.
...
In real-data mode, row-level RDS outputs and DuckDB output are disabled by
default. Disabled outputs are represented by marker files under the output
control directory.

To intentionally write row-level local outputs in a private project:

```{r enable-outputs, eval = FALSE}
Sys.setenv(ALPREKDB_WRITE_OUTPUTS = "1")
```

```{r enable-outputs-shell, eval = FALSE}
export ALPREKDB_WRITE_OUTPUTS=1
```

Even with direct PII excluded, real student-level outputs remain confidential.
...
Use `tar_destroy()` carefully. In real-data mode, `_targets/` may contain
cached row-level objects and should be treated as confidential local output.

## Inspect the Template Without Running It

The helper file can be loaded in a clean environment to inspect configuration
logic. This does not run a target graph.

```{r inspect-template-config}
...
The real-data manifest can be inspected without requiring the files to exist by
setting `validate = FALSE`. This reveals the coverage policy without reading
raw data.

```{r inspect-realdata-manifest}
...
```{r manifest-assertions, include = FALSE}
...
stopifnot(!any(canonical$module == "budget" &
  canonical$school_year == "2025-2026"))
```

## CI and Local Smoke Checks

Public CI should run only synthetic/default checks. The GitHub workflow sets
real-data mode off and uses temporary output directories.

Local template checks:

```{r template-checks, eval = FALSE}
...
Private real-data smoke checks should run only on a machine with local ADECE
source files:

```{r realdata-smoke, eval = FALSE}
Sys.setenv(
  ALPREKDB_RUN_REALDATA = "1",
  ALPREKDB_DATA_DIR = "/private/path/to/ADECE/source/files",
  ALPREKDB_WRITE_OUTPUTS = "0"
...
Do not infer, zero-fill, or copy forward 2025-26 budget records. The workflow
keeps classroom and student coverage explicit and leaves budget coverage missing
until a canonical 2025-26 budget source exists.
