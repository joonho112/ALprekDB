---
title: "M4 - Privacy and Provenance"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{M4 - Privacy and Provenance}
...
M4 documents that boundary and the provenance information that should travel
...
Ignore rules are guardrails. They reduce accidental inclusion, but users should
still review `git status` and package-build contents before any public release.

## Workflow Modes

The targets template exposes the privacy boundary through environment variables.
...
row-level RDS or DuckDB outputs unless `ALPREKDB_WRITE_OUTPUTS=1` is set.

## Local Real-Data Setup

Private workflows should keep paths in environment variables, not committed
scripts.
...
`ALPREKDB_DATA_DIR` may point to a local `ORIGINAL-DATA/` folder or another
secure source-data directory. That folder is for local processing and
provenance review only; it is excluded from git and package builds.

## Source Manifest Provenance

The private targets template keeps a source manifest. Public documentation can
describe the manifest at the module/year level without printing source
filenames.
...
The current v0.6.0 source scope is asymmetric: budget has canonical coverage
through 2024-2025, while classroom and student have canonical coverage through
2025-2026. The missing 2025-2026 budget is structural and should be recorded as
coverage metadata, not filled in.

## Privacy Defaults

Student and classroom workflows default to excluding direct private fields from
processed outputs.

```{r synthetic-privacy-check}
...
This is a structural check, not a disclosure review. Even when direct private
fields are excluded, real student-level rows remain confidential because they
can still carry sensitive combinations of attributes.
...
```{r output-opt-in, eval = FALSE}
Sys.setenv(ALPREKDB_WRITE_OUTPUTS = "1")
```

Public reports should summarize validation, coverage, and linkage diagnostics at
aggregate levels.

## Provenance Bundle for Private Outputs

A private analysis output should be accompanied by enough context to reproduce
it later. A practical bundle includes:
...
This provenance bundle can be shared more broadly than the underlying row-level
data because it contains aggregate processing context rather than records.
...
When a private run needs a diagnostic, prefer counts, status tables, coverage by
module/year, and non-disclosive validation summaries.

## Release Gate Checks

Before publishing documentation or building a release, run checks that look for
local-only artifacts.

```{r release-gate-examples, eval = FALSE}
git status --short --untracked-files=all
git check-ignore -v ORIGINAL-DATA output log _targets local.env
R CMD build --no-build-vignettes .
```

For source packages, inspect the tarball contents for ignored data folders,
local env files, database files, and rendered private logs. Ignore rules are not
a substitute for inspecting what will actually be distributed.
...
stopifnot(all(c(
  "synthetic_default",
  "realdata_default",
  "realdata_output_opt_in"
) %in% config_summary$mode))
stopifnot(config_summary$write_outputs[config_summary$mode == "synthetic_default"])
stopifnot(!config_summary$write_outputs[config_summary$mode == "realdata_default"])
stopifnot(config_summary$write_outputs[config_summary$mode == "realdata_output_opt_in"])
stopifnot(all(config_summary$include_pii == FALSE))
stopifnot(sum(canonical$module == "budget") == 4L)
stopifnot(sum(canonical$module == "classroom") == 5L)
stopifnot(sum(canonical$module == "student") == 5L)
stopifnot(!any(canonical$module == "budget" & canonical$school_year == "2025-2026"))
stopifnot(any(canonical$module == "classroom" & canonical$school_year == "2025-2026"))
stopifnot(any(canonical$module == "student" & canonical$school_year == "2025-2026"))
```
